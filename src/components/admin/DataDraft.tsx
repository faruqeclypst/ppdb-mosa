...
import { exportDraftToExcel, exportCombinedToExcel, getStatusKelengkapan } from './utils/exportExcel';
...
const DataDraft: React.FC<DataDraftProps> = ({ mode = 'regular' }) => {
  ...
  const handleConfirmForceSubmit = async () => {
    ...
    try {
      ...
      showAlert('success', `Data draft ${selectedData.namaSiswa || 'pendaftar'} berhasil diselesaikan paksa! No. Reg: ${regNumber}`);
      setShowForceSubmitModal(false);
      setSelectedData(null);
    } catch (error) {
      console.error('Error force completing draft:', error);
      showAlert('error', 'Gagal menyelesaikan paksa draft data.');
    } finally {
      ...
      >
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <p className="font-medium text-gray-900 text-sm mb-0.5 truncate">{item.namaSiswa || 'Draft Kosong'}</p>
            <p className="text-xs text-gray-500 truncate">{item.nisn ? `NISN: ${item.nisn}` : '-'}</p>
          </div>
        </div>
        ...
        <div className="px-3 pb-3 space-y-3 bg-gray-50">
          ...
          <div className="grid grid-cols-2 gap-1.5">
            ...
            <Button
              ...
              className="flex items-center justify-center gap-1 bg-emerald-50 hover:bg-emerald-100 text-emerald-600 py-2 rounded-lg text-xs transition-colors col-span-2"
            >
    ...
    <div className="space-y-6 w-full font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-zinc-200">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-amber-50 text-amber-800 border border-amber-200">
              {getFilteredData().length} Draft
            </span>
          </div>
          <p className="text-xs text-zinc-500 mt-0.5">
            {mode === 'pjj' 
              ? 'Monitoring formulir pendaftaran PJJ yang belum diselesaikan (in-progress)' 
              : 'Monitoring formulir pendaftaran reguler yang masih dalam proses pengisian'}
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            title="Export Data Draft ke Excel"
          >
      ...
      <div className="rounded-3xl p-1 bg-gradient-to-b from-white to-zinc-50 border border-zinc-200/80 shadow-sm overflow-hidden space-y-4">
        <div className="p-5 bg-white rounded-[calc(1.5rem-0.25rem)] space-y-4">
          ...
          {showFilterDropdown && (
            <div className="p-4 bg-zinc-50/80 rounded-2xl border border-zinc-200/80 space-y-3 animate-in fade-in duration-150">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-[11px] font-bold uppercase tracking-wider text-zinc-500 mb-1">Urutan Waktu</label>
                  <select
                    value={sortBy}
          ...
          {getFilteredData().length > 0 ? (
            <>
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-zinc-50/90 text-[11px] font-extrabold uppercase tracking-wider text-zinc-500 border-b border-zinc-200">
                      ...
                      <th className="px-4 py-3 min-w-[200px]">Calon Siswa</th>
                      {mode === 'pjj' && <th className="px-4 py-3 min-w-[180px]">Sekolah Mitra PJJ</th>}
                      <th className="px-4 py-3 min-w-[110px]">Jalur</th>
                      <th className="px-4 py-3 min-w-[180px]">Asal Sekolah</th>
                      <th className="px-4 py-3 min-w-[140px]">Kelengkapan</th>
                      <th className="px-4 py-3 min-w-[100px]">Status</th>
                      <th className="px-4 py-3 min-w-[150px]">Waktu Pembuatan</th>
                      <th className="px-4 py-3 min-w-[150px] text-right sticky right-0 bg-zinc-50/95 backdrop-blur-xs shadow-[-6px_0_10px_-4px_rgba(0,0,0,0.05)]">
                        Aksi Cepat
                      </th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-zinc-100 text-xs">
                    {getPaginatedData().map((item, index) => {
                      ...
                      return (
                        <tr key={item.uid} className="hover:bg-amber-50/20 transition-colors group">
                          {/* 1. NO */}
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div className="min-w-0">
                                <button
                                  ...
                                  className="font-extrabold text-zinc-900 hover:text-amber-700 transition-colors text-left block truncate max-w-[180px]"
                                  title={item.namaSiswa || 'Draft Belum Lengkap'}
                                >
                                  {item.namaSiswa || 'Draft Belum Lengkap'}
                                </button>
                          ...
                          {/* 4. JALUR */}
                          <td className="px-4 py-3">
                            {item.jalur ? <JalurBadge jalur={item.jalur} /> : <span className="text-zinc-400">-</span>}
                          </td>
                          ...
                          <td className="px-4 py-3">
                            <button
                              className="text-xs font-bold text-amber-800 bg-amber-50 hover:bg-amber-100 border border-amber-200 px-2 py-0.5 rounded-md transition-colors shadow-2xs"
                              onClick={() => {
                                setSelectedData(item);
                                setShowDetailModal(true);
                              }}
                          ...
                          {/* 9. AKSI CEPAT */}
                          <td className="px-4 py-3 text-right sticky right-0 bg-white/95 backdrop-blur-xs group-hover:bg-amber-50/80 transition-colors shadow-[-6px_0_10px_-4px_rgba(0,0,0,0.05)]">
                            <div className="flex items-center justify-end gap-1">
                              {/* Direct WhatsApp Trigger */}
                              {hasPhone && (
                                <a
                                  href={`https://wa.me/${waFormatted}?text=Halo%20${encodeURIComponent(item.namaSiswa || '')},%20terkait%20kelanjutan%20pengisian%20formulir%20SPMB...`}
                                  target="_blank"
                                  ...
                                  title="Ingatkan Calon Siswa via WhatsApp"
                                >
                              ...
                              <button
                                ...
                                className="p-1.5 rounded-lg bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-200/80 transition-colors shadow-2xs"
                                title="Selesaikan Paksa & Terbitkan No Peserta"
                              >
                              ...
                              <button
                                ...
                                className="p-1.5 rounded-lg bg-rose-50 hover:bg-rose-100 text-rose-600 border border-rose-200/80 transition-colors shadow-2xs"
                                title="Hapus Draft"
                              >
              ...
              <div className="pt-3 border-t border-zinc-100 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-zinc-500">
                <div>
                  Menampilkan <span className="font-bold text-zinc-800">{((currentPage - 1) * itemsPerPage) + 1}</span> - <span className="font-bold text-zinc-800">{Math.min(currentPage * itemsPerPage, getFilteredData().length)}</span> dari <span className="font-bold text-zinc-800">{getFilteredData().length}</span> draft pendaftar
                </div>
            ...
            <div className="p-12 text-center bg-zinc-50/60 rounded-2xl border border-zinc-100">
              <div className="max-w-sm mx-auto space-y-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-zinc-500 mt-0.5">
                    Tidak ada pendaftar yang belum menyelesaikan formulir pendaftaran
                  </p>
                </div>
                <button
                  onClick={() => {
